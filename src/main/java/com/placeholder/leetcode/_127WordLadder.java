...
public class _127WordLadder {
    ...
    private static boolean isNextWord(String word, String nextWord) {
        ...
        return letterChanged == 1;
    }

    private static Map<String, Set<String>> buildTransformMap(Set<String> wordList) {
        Map<String, Set<String>> transformMap = new HashMap<>();
        for (String candidate : wordList) {
            transformMap.put(candidate, new HashSet<>());
        }

        for (String candidate1 : wordList) {
            wordList.stream()
                    .filter(candidate2 -> isNextWord(candidate1, candidate2))
                    .forEach(candidate2 -> transformMap.get(candidate1).add(candidate2));
        }
        return transformMap;
    }

    // 超时
    public static int dfsLadderLength(String beginWord, String endWord, Set<String> wordList) {

        wordList.add(beginWord);
        wordList.add(endWord);
        Map<String, Set<String>> transformMap = buildTransformMap(wordList);

        List<String> path = new ArrayList<>();
        path.add(beginWord);

        int[] minPathLength = new int[1];
        minPathLength[0] = Integer.MAX_VALUE;
        dfs(transformMap, endWord, beginWord, path, minPathLength);
        return minPathLength[0];
    }

    private static void dfs(Map<String, Set<String>> transformMap,
                            String endWord,
                            String currentWord,
                            List<String> path,
                            int[] minPathLength) {
        if (path.size() >= minPathLength[0]) { // 剪枝
            return;
        }
        if (endWord.equals(currentWord)) {
            minPathLength[0] = path.size();
            return;
        }
        Set<String> nextWords = transformMap.get(currentWord);
        nextWords.stream()
                .filter(nextWord -> !path.contains(nextWord)) // ignore replicated
                .forEach(nextWord -> {
                    path.add(nextWord);
                    dfs(transformMap, endWord, nextWord, path, minPathLength);
                    path.remove(path.size() - 1);
                });
    }

    public static int bfsLadderLength(String beginWord, String endWord, Set<String> wordList) {

        wordList.add(beginWord);
        wordList.add(endWord);
        Map<String, Set<String>> transformMap = buildTransformMap(wordList);
        return bfs(transformMap, beginWord, endWord);
    }

    private static int bfs(Map<String, Set<String>> transformMap,
                           String beginWord,
                           String endWord) {
        ...
        while (!wordQueue.isEmpty()) {
            ...
            transformMap.get(currentWord).stream()
    ...
    // 超时
    public static int dijkstraLadderLength(String beginWord, String endWord, Set<String> wordList) {
        wordList.add(beginWord);
        wordList.add(endWord);
        List<Vertex> vertices = buildGraph(wordList);
        Vertex beginVertex = null, endVertex = null;
        for (Vertex vertice : vertices) {
            if (vertice.word.equals(beginWord)) {
                beginVertex = vertice;
            }
            if (vertice.word.equals(endWord)) {
                endVertex = vertice;
            }
        }
        return dijkstra(beginVertex, endVertex, vertices) + 1;
    }

    private static List<Vertex> buildGraph(Set<String> wordList) {
        List<Vertex> vertices = new ArrayList<>();
        for (String s1 : wordList) {
            Vertex vertex = new Vertex(s1);
            vertices.add(vertex);
        }

        for (Vertex vertex1 : vertices) {
            for (Vertex vertex2 : vertices) {
                if (isNextWord(vertex1.word, vertex2.word)) {
                    vertex1.addNext(vertex2);
                }
            }
        }
        return vertices;
    ...
    // http://www.cs.yale.edu/homes/aspnes/pinewiki/ShortestPath.html
    // 构建状态转移图，找最短路径
    private static int dijkstra(Vertex start, Vertex end, List<Vertex> graph) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (minVertex != null) {
                ...
                Set<Vertex> minVertexNext = minVertex.next;
                if (minVertexNext == null)
                    continue;

                // for each each uv:
                for (Vertex next : minVertexNext) {
                    //  d[v] = min(d[v], d[u] + w(u,v))
                    distances.put(next, Math.min(distances.get(next), distances.get(minVertex) + 1));
                }
            }
        }
    ...
    static class Vertex {
        ...
        public void addNext(Vertex next) {
            if (this.next == null)
